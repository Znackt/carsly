...
interface CreateBookingProps {
  companyId: string | number; // Passed dynamically from your page
  onClose: () => void;
  onSave?: (data: any) => void;
}

const CreateBooking = ({ companyId, onClose, onSave }: CreateBookingProps) => {
  // --- PRODUCTION STATE: API DATA ---
  const [customers, setCustomers] = useState<Customer[]>([]);
  const [packages, setPackages] = useState<ServicePackage[]>([]);
  const [locations, setLocations] = useState<Location[]>([]);
  
  // --- PRODUCTION STATE: UI CONTROL ---
  ...
  if (isFetchingData) {
    return (
      <div className="fixed inset-0 bg-black/20 flex items-center justify-center z-50">
        <div className="bg-white p-8 rounded-2xl flex flex-col items-center shadow-lg">
          <Loader2 className="animate-spin text-[#3b5bdb] mb-4" size={32} />
          <p className="text-sm font-medium text-gray-700">Loading database records...</p>
        </div>
      </div>
    );
  }
  ...
  if (fetchError) {
    return (
      <div className="fixed inset-0 bg-black/20 flex items-center justify-center z-50">
        <div className="bg-white p-8 rounded-2xl flex flex-col items-center shadow-lg">
          <p className="text-red-500 mb-4 font-medium">{fetchError}</p>
